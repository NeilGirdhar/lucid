...
"#,
    ...
    #[test]
    fn result_abi_executes_range_accumulation_with_local_step_literal_alias_cfg() {
        let module = lucid_syntax::parse(
            r#"total = 0
stride = -1
...
"#,
        ...
        let compiled = compile_integer_result_function(&function)
            .expect("result ABI should compile range step literal alias loop CFG");
...
"#,
    ...
    #[test]
    fn result_abi_executes_local_init_local_induction_step_while_accumulation_cfg() {
        let module = lucid_syntax::parse(
            r#"value = n
tick = step
total = 0
...
"#,
        ...
        .expect("local-init local-induction-step while accumulation fixture should parse");
        let function = lucid_cir::Function::from_module_linear_with_params(
            &module,
            &["n".into(), "step".into()],
        )
        .expect("local-init local-induction-step while accumulation should lower");
        let compiled = compile_integer_result_function(&function).expect(
            "result ABI should compile local-init local-induction-step while accumulation CFG",
        );
...
"#,
    ...
    #[test]
    fn result_abi_executes_local_bound_parameter_induction_counted_while_cfg() {
        let module = lucid_syntax::parse(
            r#"stop = limit
while n > stop:
    n -= 1
...
"#,
        ...
        .expect("local-bound parameter-induction counted while fixture should parse");
        let function = lucid_cir::Function::from_module_linear_with_params(
            &module,
            &["n".into(), "limit".into()],
        )
        .expect("local-bound parameter-induction counted while should lower");
        let compiled = compile_integer_result_function(&function)
...
"#,
    ...
    #[test]
    fn result_abi_executes_local_division_bound_counted_while_cfg() {
        let module = lucid_syntax::parse(
            r#"stop = limit // scale
while n > stop:
    n -= 1
...
"#,
        ...
        .expect("local division-bound counted while fixture should parse");
        let function = lucid_cir::Function::from_module_linear_with_params(
            &module,
            &["n".into(), "limit".into(), "scale".into()],
        )
        .expect("local division-bound counted while should lower");
        let compiled = compile_integer_result_function(&function)
            .expect("result ABI should compile local division-bound counted loop CFG");
        let result = unsafe { compiled.call_result_with_args(&[5, 6, 2]) };
        assert!(result.is_ok());
        assert_eq!(result.value, 3);
        let zero = unsafe { compiled.call_result_with_args(&[5, 6, 0]) };
...
"#,
    ...
    #[test]
    fn result_abi_executes_local_division_bound_counted_while_accumulator_cfg() {
        let module = lucid_syntax::parse(
            r#"stop = limit // scale
total = 0
while n > stop:
...
"#,
        ...
        .expect("local division-bound counted while accumulator fixture should parse");
        let function = lucid_cir::Function::from_module_linear_with_params(
            &module,
            &["n".into(), "limit".into(), "scale".into()],
        )
        .expect("local division-bound counted while accumulator should lower");
        let compiled = compile_integer_result_function(&function)
            .expect("result ABI should compile local division-bound counted accumulator CFG");
        let result = unsafe { compiled.call_result_with_args(&[5, 6, 2]) };
        assert!(result.is_ok());
        assert_eq!(result.value, 9);
        let zero = unsafe { compiled.call_result_with_args(&[5, 6, 0]) };
...
"#,
        ...
        .expect("local-bound local-step parameter-induction counted while fixture should parse");
        let function = lucid_cir::Function::from_module_linear_with_params(
            &module,
            &["n".into(), "limit".into(), "step".into()],
        )
        .expect("local-bound local-step parameter-induction counted while should lower");
        let compiled = compile_integer_result_function(&function).expect(
            "result ABI should compile local-bound local-step parameter-induction counted loop CFG",
        );
        let result = unsafe { compiled.call_result_with_args(&[10, 2, 3]) };
        assert!(result.is_ok());
    ...
    #[test]
    fn result_abi_executes_local_bound_local_step_counted_while_cfg() {
        let module = lucid_syntax::parse(
            r#"value = n
stop = limit
tick = step
...
"#,
        ...
        .expect("local-bound local-step counted while fixture should parse");
        let function = lucid_cir::Function::from_module_linear_with_params(
            &module,
            &["n".into(), "limit".into(), "step".into()],
        )
        .expect("local-bound local-step counted while should lower");
        let compiled = compile_integer_result_function(&function)
            .expect("result ABI should compile local-bound local-step counted while CFG");
        let result = unsafe { compiled.call_result_with_args(&[10, 2, 3]) };
        assert!(result.is_ok());
        assert_eq!(result.value, 1);
    }
    ...
    #[test]
    fn result_abi_executes_dynamic_arithmetic_step_counted_while_cfg() {
        let module = lucid_syntax::parse(
            r#"tick = step + 1
while n > 0:
    n -= tick
...
"#,
        ...
        .expect("dynamic arithmetic-step counted while fixture should parse");
        let function = lucid_cir::Function::from_module_linear_with_params(
            &module,
            &["n".into(), "step".into()],
        )
        .expect("dynamic arithmetic-step counted while should lower");
        let compiled = compile_integer_result_function(&function)
            .expect("result ABI should compile dynamic arithmetic-step counted while CFG");
        let result = unsafe { compiled.call_result_with_args(&[10, 3]) };
        assert!(result.is_ok());
        assert_eq!(result.value, -2);
    }
...
"#,
    ...
    #[test]
    fn result_abi_executes_local_constant_induction_while_accumulation_cfg() {
        let module = lucid_syntax::parse(
            r#"total = 0
value = 1 + 2
while value > 0:
...
"#,
        ...
        .expect("local constant-induction while accumulation fixture should parse");
        let function = lucid_cir::Function::from_module_linear_with_params(&module, &[])
            .expect("local constant-induction while accumulation should lower");
        let compiled = compile_integer_result_function(&function)
            .expect("result ABI should compile local constant-induction while accumulation CFG");
        let result = unsafe { compiled.call_result_with_args(&[]) };
        assert!(result.is_ok());
        assert_eq!(result.value, 6);
    }
...
"#,
    ...
    #[test]
    fn result_abi_executes_parameter_induction_local_bound_while_accumulation_cfg() {
        let module = lucid_syntax::parse(
            r#"total = 0
stop = limit
while n > stop:
...
"#,
        ...
        .expect("parameter-induction local-bound while accumulation fixture should parse");
        let function = lucid_cir::Function::from_module_linear_with_params(
            &module,
            &["n".into(), "limit".into()],
        )
        .expect("parameter-induction local-bound while accumulation should lower");
        let compiled = compile_integer_result_function(&function).expect(
    ...
    #[test]
    fn result_abi_executes_parameter_induction_local_constant_bound_while_accumulation_cfg() {
        let module = lucid_syntax::parse(
            r#"total = 0
stop = 1 + 1
while n > stop:
...
"#,
        ...
        .expect("parameter-induction local constant-bound while accumulation fixture should parse");
        let function = lucid_cir::Function::from_module_linear_with_params(&module, &["n".into()])
            .expect("parameter-induction local constant-bound while accumulation should lower");
        let compiled = compile_integer_result_function(&function).expect(
            "result ABI should compile parameter-induction local constant-bound while accumulation CFG",
        );
        let result = unsafe { compiled.call_result_with_args(&[5]) };
        assert!(result.is_ok());
        assert_eq!(result.value, 12);
...
"#,
    ...
    #[test]
    fn result_abi_executes_local_bound_local_step_while_accumulation_cfg() {
        let module = lucid_syntax::parse(
            r#"value = n
stop = limit
tick = step
...
"#,
        ...
        .expect("local-bound local-step while accumulation fixture should parse");
        let function = lucid_cir::Function::from_module_linear_with_params(
            &module,
            &["n".into(), "limit".into(), "step".into()],
        )
        .expect("local-bound local-step while accumulation should lower");
        let compiled = compile_integer_result_function(&function)
            .expect("result ABI should compile local-bound local-step while accumulation CFG");
        let result = unsafe { compiled.call_result_with_args(&[5, 2, 3]) };
        assert!(result.is_ok());
        assert_eq!(result.value, 9);
    }

    #[test]
    fn result_abi_executes_local_bound_local_induction_step_while_accumulation_cfg() {
        let module = lucid_syntax::parse(
            r#"value = n
stop = limit
tick = step
...
"#,
        ...
        .expect("local-bound local-induction-step while accumulation fixture should parse");
        let function = lucid_cir::Function::from_module_linear_with_params(
            &module,
            &["n".into(), "limit".into(), "step".into()],
        )
        .expect("local-bound local-induction-step while accumulation should lower");
        let compiled = compile_integer_result_function(&function).expect(
            "result ABI should compile local-bound local-induction-step while accumulation CFG",
        );
        let result = unsafe { compiled.call_result_with_args(&[10, 2, 3]) };
        assert!(result.is_ok());
    ...
    #[test]
    fn result_abi_executes_parameter_induction_local_constant_step_while_accumulation_cfg() {
        let module = lucid_syntax::parse(
            r#"total = 0
tick = 1 + 1
while n > 0:
...
"#,
        ...
        .expect("parameter-induction local constant-step while accumulation fixture should parse");
        let function = lucid_cir::Function::from_module_linear_with_params(&module, &["n".into()])
            .expect("parameter-induction local constant-step while accumulation should lower");
        let compiled = compile_integer_result_function(&function).expect(
            "result ABI should compile parameter-induction local constant-step while accumulation CFG",
        );
        let result = unsafe { compiled.call_result_with_args(&[5]) };
        assert!(result.is_ok());
        assert_eq!(result.value, 9);
...
"#,
    ...
    #[test]
    fn result_abi_propagates_division_errors_through_branch_local_phi() {
        let function = Function {
            entry: lucid_cir::BlockId(0),
            blocks: vec![
                lucid_cir::Block {
        ...
        let compiled = compile_integer_result_function(&function)
            .expect("branch-local division phi should use the result ABI");
        assert_eq!(
            unsafe { compiled.call_result_with_args(&[1, 42, 2]) },
            crate::native_abi::NativeResult::ok(21)
        );
