...
impl Function {
    ...
    fn from_counted_while(
        ...
        fn operand_instruction(
            ...
            if let Some((instruction, used_alias)) =
                atom_instruction(expr, result, parameter_names, step_initial)
            {
                return Some((vec![instruction], used_alias));
            }
            match expr {
                lucid_syntax::Expr::Ident { name, .. } => {
                    let statement = step_initial?;
            ...
            .as_slice()
        {
            [while_statement, lucid_syntax::Stmt::Return {
                ...
                ..
            }] => (None, None, None, while_statement, Some(name)),
            [while_statement] => (None, None, None, while_statement, None),
            [initial, while_statement, lucid_syntax::Stmt::Return {
                value: Some(lucid_syntax::Expr::Ident { name, .. }),
                ..
            }] => (Some(initial), None, None, while_statement, Some(name)),
            [initial, while_statement] => (Some(initial), None, None, while_statement, None),
            [first_statement, second_statement, while_statement] => {
                ...
                match right.as_ref() {
                    ...
                    .is_some() =>
                    {
                        ...
                        } else if second_name == induction_name && first_name != induction_name {
                            (
                                Some(second_statement),
                                None,
                                Some(first_statement),
    ...
    pub fn from_module_linear_with_params(
        ...
        fn lower_dynamic_if_elif_ladder(
            ...
            let (merge_instructions, merge_last) = if ladder.suffix.is_empty() {
                let then_value = then_lowering
                    .value
                    .ok_or(LowerError::NoLowerableAssignment)?;
                let mut incomings = vec![(BlockId(1), then_value)];
                for (index, (_, _, branch)) in lowered_elifs.iter().enumerate() {
                    let arm_block = BlockId(
                        3 + u32::try_from(index).map_err(|_| LowerError::UnsupportedExpression)?
                            * 2,
                    );
                    incomings.push((
                        arm_block,
                        branch.value.ok_or(LowerError::NoLowerableAssignment)?,
                    ));
                }
                incomings.push((
                    fallback_block,
...
"#,
        ...
        .expect("dynamic arithmetic-step counted loop fixture should parse");
        let function =
            Function::from_module_linear_with_params(&module, &["n".into(), "step".into()])
                .expect("dynamic arithmetic-step counted loop should lower");
        assert_eq!(function.execute_with_args(&[10, 3]), Ok(Some(-2)));
...
"#,
        ...
        .expect("local dynamic arithmetic-step counted loop fixture should parse");
        let function =
            Function::from_module_linear_with_params(&module, &["n".into(), "step".into()])
...
"#,
        ...
        .expect("local constant-step counted loop fixture should parse");
        let function = Function::from_module_linear_with_params(&module, &["n".into()])
            .expect("local constant-step counted loop should lower");
...
"#,
        ...
        .expect("local-init local-step counted loop fixture should parse");
        let function =
            Function::from_module_linear_with_params(&module, &["n".into(), "step".into()])
                .expect("local-init local-step counted loop should lower");
        assert_eq!(function.execute_with_args(&[10, 3]), Ok(Some(-2)));
...
"#,
        ...
        .expect("reordered local-init local-step counted loop fixture should parse");
        let function =
            Function::from_module_linear_with_params(&module, &["n".into(), "step".into()])
                .expect("reordered local-init local-step counted loop should lower");
        assert_eq!(function.execute_with_args(&[10, 3]), Ok(Some(-2)));
...
"#,
        ...
        .expect("not-comparison guarded while accumulator fixture should parse");
        let function = Function::from_module_linear_with_params(
            &module,
            &["n".into(), "high".into(), "low".into()],
        )
        .expect("not-comparison guarded while accumulator should lower through CIR");
...
"#,
        ...
        .expect("guarded while accumulator elif fixture should parse");
        let function = Function::from_module_linear_with_params(
            &module,
            &["n".into(), "high".into(), "low".into()],
        )
        .expect("guarded while accumulator elif should lower through CIR");
...
"#,
        ...
        .expect("range boolean-guard accumulation fixture should parse");
        let function = Function::from_module_linear_with_params(
            &module,
            &["limit".into(), "low".into(), "high".into()],
        )
...
"#,
    ...
    #[test]
    fn parameterized_mixed_ladders_short_circuit_logical_guards() {
        fn return_expr(source: &str) -> lucid_syntax::Expr {
            let module = lucid_syntax::parse(&format!("return {source}\n"))
                .expect("expression fixture should parse");
            let [lucid_syntax::Stmt::Return {
                value: Some(expr), ..
            }] = module.statements.as_slice()
            else {
                panic!("expected single return expression");
            };
            expr.clone()
        }

        let parameters = vec!["x".to_string(), "y".to_string()];
        let initial_guard = return_expr("x != 0 and 10 // x > 1");
        let elif_guard = return_expr("y != 0 and 10 // y > 1");
        let one = return_expr("1");
        let two = return_expr("2");
        let three = return_expr("3");

        let void_then_value_else = Function::from_parameterized_if_void_elif_chain_direct(
            &initial_guard,
            &[(&elif_guard, &two)],
            &three,
            &parameters,
        )
        .expect("void/value direct ladder should lower");
        assert_eq!(void_then_value_else.execute_with_args(&[0, 0]), Ok(Some(3)));
        assert_eq!(void_then_value_else.execute_with_args(&[0, 2]), Ok(Some(2)));

        let void_then_value_optional = Function::from_parameterized_if_void_elif_optional_chain(
            &initial_guard,
            &[(&elif_guard, &two)],
            &parameters,
        )
        .expect("void/value optional ladder should lower");
        assert_eq!(
            void_then_value_optional.execute_with_args(&[0, 0]),
            Ok(None)
        );
        assert_eq!(
            void_then_value_optional.execute_with_args(&[0, 2]),
            Ok(Some(2))
        );

        let value_then_void_else = Function::from_parameterized_if_elif_voids_else_direct(
            &initial_guard,
            &one,
            &[&elif_guard],
            &three,
        ...
        .expect("value/void direct ladder should lower");
        assert_eq!(value_then_void_else.execute_with_args(&[0, 0]), Ok(Some(3)));
        assert_eq!(value_then_void_else.execute_with_args(&[2, 0]), Ok(Some(1)));
        assert_eq!(value_then_void_else.execute_with_args(&[0, 2]), Ok(None));

        let value_then_void_optional = Function::from_parameterized_if_elif_voids_optional(
            &initial_guard,
            &one,
            &[&elif_guard],
            &parameters,
        )
        .expect("value/void optional ladder should lower");
        assert_eq!(
            value_then_void_optional.execute_with_args(&[0, 0]),
            Ok(None)
        );
        assert_eq!(
            value_then_void_optional.execute_with_args(&[2, 0]),
            Ok(Some(1))
        );
        assert_eq!(
            value_then_void_optional.execute_with_args(&[0, 2]),
            Ok(None)
        );
    }
    ...
    fn linear_module_lowering_lowers_constant_string_predicates() {
        for (source, expected) in [
            ...
            (
                "values = {\"a\": \"b\"}\nkey = \"a\"\nreturn key in values and values[key] == \"b\"\n",
                1,
            ),
            (
                "values = {\"a\": 1}\nkey = \"a\"\nreturn len(values) == 1 and key in values and values[key] == 1\n",
                1,
            ),
            (
                "values = {1: \"a\"}\nreturn len(values) == 1 and 1 in values and values[1] == \"a\"\n",
                1,
            ),
    ...
    #[test]
    fn linear_module_lowering_lowers_constant_float_predicates() {
        for (source, expected) in [
            ("return 1.5 == 1.5\n", 1),
            ("return 1.5 != 2.5\n", 1),
            ("return 1.5 < 2.5\n", 1),
    ...
    fn linear_module_lowering_lowers_constant_none_predicates() {
        for (source, expected) in [
            ...
            ("values = {1.5: None}\nreturn len(values)\n", 1),
            ("return {1.5: None} == {1.5: None}\n", 1),
            ("values = {1.5: None}\nreturn 1.5 in values\n", 1),
            ("values = {1.5: None}\nreturn values[1.5] is None\n", 1),
            ("values = {None: 1.5}\nreturn len(values)\n", 1),
    ...
    fn dynamic_if_lowering_builds_verified_phi_merge() {
        ...
        let function =
            Function::from_module_linear_with_params(&module, &["first".into(), "second".into()])
                .expect("final dynamic elif ladder should merge branch result values");
        ...
        let function =
            Function::from_module_linear_with_params(&module, &["first".into(), "second".into()])
                .expect("final no-else dynamic elif ladder should merge initialized fallback");
        assert_eq!(function.execute_with_args(&[1, 1]), Ok(Some(10)));
        assert_eq!(function.execute_with_args(&[0, 1]), Ok(Some(20)));
        assert_eq!(function.execute_with_args(&[0, 0]), Ok(Some(40)));
        let module = lucid_syntax::parse(
