...
fn run_cir_lowers_constant_float_predicates_in_typed_function_body() {
    ...
    fs::write(
        &path,
        "def answer():\n    value = 1.5\n    empty = 0.0\n    values = [1.5, 2.5]\n    tuple_values = (1.5, 2.5)\n    tuple_list = list(tuple_values)\n    tuple_set = set(tuple_values)\n    tuple_sorted = sorted((2.5, 1.5))\n    tuple_reversed = reversed(tuple_values)\n    lookup = {1.5: 2.5}\n    selected = 1 if value else 0\n    if value < 2.5:\n        return selected and bool(value) and not bool(empty) and value != 2.5 and len(values) == 2 and values[1] == 2.5 and value in values and lookup[value] == 2.5 and 2.5 in tuple_values and 3.5 not in tuple_values and tuple_list[1] == 2.5 and 2.5 in tuple_set and tuple_sorted[0] == 1.5 and tuple_reversed[0] == 2.5\n    else:\n        return false\n",
    )
    .expect("temporary source should be writable");
    let output = Command::new(env!("CARGO_BIN_EXE_lucid"))
        .args([
