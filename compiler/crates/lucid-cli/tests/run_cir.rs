...
fn run_cir_lowers_len_of_constant_aggregates_in_typed_function_body() {
    ...
    fs::write(
        &path,
        "def answer():\n    values = [1, 2, 3]\n    unique = {4, 5, 6}\n    pairs = {1: 10, 2: 20}\n    return len(values) * 10 + len(unique) + len(pairs) + len([4, 5])\n",
    )
    .expect("temporary source should be writable");
    let output = Command::new(env!("CARGO_BIN_EXE_lucid"))
        .args([
    ...
    assert_eq!(String::from_utf8_lossy(&output.stdout).trim(), "37");
}
...
fn run_cir_lowers_constant_integer_builtins_in_typed_function_body() {
    ...
    fs::write(
        &path,
        "def answer():\n    values = [1, 2]\n    flags = {false, 0, 2}\n    numbers = range(5)\n    empty = range(0)\n    positives = range(1, 4)\n    data = b\"abc\"\n    record = (1,)\n    label = \"42\"\n    return all(values) and any(flags) and bool(range(3)) and not bool([]) and bool(data) and bool(record) and int(true) == 1 and int(\"42\") == 42 and int(label) == 42 and int(1 < 2) == 1 and 3 in numbers and 7 not in numbers and 98 in data and data[1] == 98 and 120 not in data and sum(numbers) == 10 and len(numbers) == 5 and not bool(empty) and all(positives) and not any(empty)\n",
    )
    .expect("temporary source should be writable");
    let output = Command::new(env!("CARGO_BIN_EXE_lucid"))
        .args([
...
fn run_cir_lowers_constant_string_predicates_in_typed_function_body() {
    ...
    fs::write(
        &path,
        "def answer():\n    text = \"lucid\"\n    padded = \"  Lucid  \"\n    parts = \"lu,cid\".split(\",\")\n    needle = \"u\"\n    missing = \"z\"\n    score = len(text) if needle in text else 1 // 0\n    return score == 5 and bool(text) and text[1] == \"u\" and missing not in text and text.startswith(\"lu\") and text.endswith(\"id\") and padded.strip().lower() == text and text.upper() == \"LUCID\" and len(parts) == 2 and \"\".join(parts) == text and \"cid\" in parts and \"z\" not in parts\n",
    )
    .expect("temporary source should be writable");
    let output = Command::new(env!("CARGO_BIN_EXE_lucid"))
        .args([
...
fn run_cir_lowers_constant_none_predicates_in_typed_function_body() {
    ...
    fs::write(
        &path,
        "def answer():\n    missing = None\n    marker = ...\n    values = [None, ...]\n    lookup = {None: None}\n    string_lookup = {\"a\": None}\n    singleton_lookup = {None: \"a\"}\n    int_lookup = {1: None}\n    singleton_int_lookup = {None: 1}\n    return not bool(missing) and bool(marker) and None == None and None is None and not (None != None) and not (None is not None) and marker is ... and None is not marker and len(values) == 2 and values[0] is None and marker in values and lookup[None] is None and len(string_lookup) == 1 and \"a\" in string_lookup and string_lookup[\"a\"] is None and len(singleton_lookup) == 1 and None in singleton_lookup and singleton_lookup[None] == \"a\" and len(int_lookup) == 1 and 1 in int_lookup and int_lookup[1] is None and len(singleton_int_lookup) == 1 and None in singleton_int_lookup and singleton_int_lookup[None] == 1\n",
    )
    .expect("temporary source should be writable");
    let output = Command::new(env!("CARGO_BIN_EXE_lucid"))
        .args([
