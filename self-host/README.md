...
- `compile_demo.lucid` — **an actual compiler, written in Lucid,
  ...
  four hand-written programs (recursion, iteration,
  `%`/`and`/`or`/comparisons, and `//`/`%`'s exact Euclidean semantics on
  all four sign combinations) and one that's supposed to fail checking,
  proving a real error stops codegen instead of emitting broken C. Each
  valid program's C output is written to
  ...
  `freeze()` and `for`/`if_broken`; `examples/collections.lucid` needs
  comprehensions, ranges, dicts, and sets; `examples/errors.lucid` needs
  `?` compiled to some checked-error ABI; `examples/hello.lucid` needs
  string methods and comprehensions — none of which
