...
- `compile_demo.lucid` — **an actual compiler, written in Lucid,
  ...
  five hand-written programs (recursion, iteration,
  `%`/`and`/`or`/comparisons, `//`/`%`'s exact Euclidean semantics on all
  four sign combinations, and — `loops` — `for` over both a list literal
  and `range(...)`, a nested `for` whose inner loop's `if_broken` clause
  re-breaks the outer loop, and a `while` with its own `if_broken`) and
  one that's supposed to fail checking, proving a real error stops
