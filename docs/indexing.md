...
marker (see [No tuple or namedtuple type](collections.md)):
...
types (`T` versus `list[T]`), the same reason [No @overload](dispatch.md) replaces `@overload` with `dispatch` rather than a
...
[No tuple or namedtuple type](collections.md).
