...
[Library initialization](#library-initialization) below), so a project's dependency list and its
...
```
Only a name that is not itself [module- or class-private](modules.md)
...
exactly the internal layout [Public API](#public-api) already keeps out of everything
...
[Mutable, read-only, and immutable views](mutability.md)) because a
...
[Context managers](context-managers.md)), holding the setup work that
...
It is committed for a project with [Entry points](#entry-points) — an application,
