...
and immutable views (`Mutable, read-only, and immutable views
<mutability.rst>`__): whatever provides at least the capability asked
...
(see `Final fields <classes.rst>`_). A ``getter`` does not satisfy it,
...
type, using ``F[_]`` (see
`Higher-kinded parameters <generics.rst>`_). Inside such a trait, ``Self``
