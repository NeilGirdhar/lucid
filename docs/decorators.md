...
([Gather](gather.md)) instead of one-to-one, so
...
[function type's](types.md) left side, like `P` here, stands for a whole
...
unlike [Partial application with _](calls.md) in an ordinary call, `@` already
