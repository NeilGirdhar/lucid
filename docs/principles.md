...
[Zero-deprecation](#zero-deprecation) is why Lucid keeps one way — [No tuple or namedtuple type](collections.md) picked named records once, instead of letting
...
interception hooks ([Classes](classes.md)). Nothing about a piece of
...
it declares. See [Traits](traits.md) and
[One class parent](classes.md).
...
`~T`, and `!T`. See [Generics](generics.md),
...
third parties the way [Dispatch beyond operators](dispatch.md) already
...
as an operator negotiation protocol. See [Multiple dispatch](dispatch.md).
