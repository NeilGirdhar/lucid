...
`Positional arguments precede keyword arguments <calls.rst>`_ assumes throughout its
examples.
...
what they will be (see `Decorators <decorators.rst>`_). ``Parameters`` is usable as a
...
``vpargs``/``kwargs`` are empty — exactly `Decorators <decorators.rst>`__'s ``f(***args)``
...
exactly how `Decorators <decorators.rst>`__ uses it.
