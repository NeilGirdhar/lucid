...
gone. [Freezing is deep](#freezing-is-deep) describes what every frozen value must behave
...
[Field reflection with fields](construction.md) yields a class's own fields
...
[Explicit overrides](traits.md) requires `override` before a trait
...
less-than to compare (see [Numeric types](numeric-types.md)) — and
...
`Hashable` say, for the reason [Read-only view](#read-only-view) already gave for
...
than a real second form (see [Numeric types](numeric-types.md)).
...
[Numeric types](numeric-types.md)). `set` keeps `Ord`, but gives
