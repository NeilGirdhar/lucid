...
* Generic functions retain formal type parameters and infer direct argument
  ...
  `MemoryView`, `list`, `set`, and `dict` are treated as type/class patterns,
  not new local bindings, so predeclared locals stay aligned with match
  checking.
  ...
  `case dict` now test those value kinds the same way.
