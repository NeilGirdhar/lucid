...
Type narrowing gap now ~80% complete (basic implementation working, attribute paths remain).
Remaining major gaps (~4-6 days estimated total):
- Attribute path narrowing (1-2 days): extend type narrowing to `obj.field` patterns  
- Iterator protocol (2-3 days): architectural changes for lazy evaluation, already has trait definition
...
* Primitive literal `match` functions now lower through verified CIR decision
  ...
  expression results followed by a wildcard. Optional literal chains
  without a wildcard, or with a no-op wildcard fallback such as `pass`, now use
  ...
  evaluated. The parser now accepts float, complex, and ellipsis literal match
  patterns. Constant bigint, float, complex, string, bytes, `none`, and
  ellipsis subjects use the same selected-arm path when the chosen arm can be
  lowered without carrying that value into CIR. Static guards participate in
...
* Canonical counted `while` and `for range` CIR loops now accept a trailing
  ...
  unreachable `if_broken` completion clause, including effectful suites,
  ...
  `None`; they accept the same local aliases and dynamic or static bound and
  step expressions as the accumulator form.
  ...
  the recoverable error: checked constants, parameters, local aliases, unary
  `+`/`-`, and binary `+`, `-`, `*`, `/`, `//`, or `%` combinations lower
  ...
  parameter, or the same unary/binary integer-expression subset, including
  division-family operators for the accumulator operand, using
...
* Discarded primitive expressions before value and bare returns now pass through
  ...
  CIR branch path, including optional no-`else` fall-through, void-only
  ...
  except for arithmetic expressions that can report recoverable errors, which
  lower as ordered prefixes so division and overflow errors are not erased.
  ...
  for primitive two-arm and literal-chain match results, including statically
  true `assert`, statically false `while` setup, and statically empty `for`
...
* The checker now treats `Bytes` as a distinct immutable buffer type rather
  ...
  The lexer and parser recognize `b"..."` literals, and both execution paths
  now carry immutable bytes as distinct runtime values for conversion,
...
* `MemoryView` now has an actual no-copy runtime representation in both
  interpreter and native output. Mutable bytearray-backed views share indexed
...
* A Salsa-tracked `lower_function_body` query now consumes checked function
  ...
  for one-sided conditionals, two-arm conditionals, statically false leading
  arms, and `if`/`elif` ladders: the initializer now executes before the first
  dynamic condition even when every branch overwrites the returned local, so
  recoverable initializer errors cannot be skipped by the branch shortcut.
...
* `emit-cir` and `run-cir` accept `--function NAME`, exposing the typed-HIR
  ...
  Setup statements before guard-return ladders now lower through typed
...
* A dynamic outer `if` whose selected arm initializes a local, conditionally
  ...
  It is also reused for fall-through code after an `if`/`elif` chain with no
  syntactic `else`, by normalizing the remaining statements into the fallback
  arm.
  ...
  Match statements with no final wildcard can now fall through into the same
  nested local branch shape after their direct-return literal arms.
...
* Counted `while` accumulator lowering now accepts a single guarded accumulator
  ...
  A single `elif` accumulator update is supported as well, with all selected
  accumulator values merging before the shared induction step.
