...
[Higher-kinded traits](traits.md) for that alternative,
...
fallback, not the kind of tie [Ambiguous dispatch is an error](#ambiguous-dispatch-is-an-error) describes
...
at all — the same extensibility [Dispatch across projects and hierarchies](#dispatch-across-projects-and-hierarchies)
...
mode [Ambiguous dispatch is an error](#ambiguous-dispatch-is-an-error) already rejects for ordinary
method resolution.

Lucid needs no separate mechanism for this: it is exactly
[Dispatch beyond operators](#dispatch-beyond-operators), applied to a function with no shared body
across its cases:
...
same extensibility [Dispatch across projects and hierarchies](#dispatch-across-projects-and-hierarchies) already
described.
...
ordinary value the way [Reifying a type expression](types.md#reifying-a-type-expression)
